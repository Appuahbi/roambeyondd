...
const getEnquiryById = async (enquiryId, user) => {
    ...
    if (user.role !== "admin" && (!enquiry.user || enquiry.user._id.toString() !== user.id)) {
...
const updateEnquiry = async (enquiryId, updateData) => {
    ...
    const allowedFields = ["leadStatus", "leadSource", "priority", "followUpDate", "lastContactedAt", "remarks"];
