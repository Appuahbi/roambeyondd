...
import { Mail, Phone, Calendar, UserPlus, Trash2 } from 'lucide-react';
...
const ROLE_BADGES = {
  ...
  agent: <Badge variant="brand">Agent</Badge>,
...
export default function AdminUsers() {
  ...
  const changeRole = async (u, next) => {
    ...
    if (currentUser?._id === u._id && next !== 'admin') {
        ...
        subtitle="Everyone who has signed up. Promote or demote admins here."
          ...
          <div className="flex items-center gap-2">
            <select value={role} onChange={(e) => setRole(e.target.value)} className="input !w-auto">
              ...
              {ROLES.map((r) => <option key={r} value={r}>{r[0].toUpperCase() + r.slice(1)}s</option>)}
            </select>
          ...
          columns={[
{
            ...
            { key: 'role', label: 'Role', render: (r) => ROLE_BADGES[r.role] || ROLE_BADGES.user },
            { key: 'createdAt', label: 'Joined', render: (r) => formatDate(r.createdAt) },
            {
              ...
              render: (r) => (
                <div className="inline-flex items-center gap-1">
                  <select
                    value={r.role}
                    disabled={busyId === r._id || currentUser?._id === r._id}
