  ...
  return (
    <div className="min-h-[72vh] grid place-items-center px-4 bg-cream-gradient relative overflow-hidden">
      ...
      <div className="relative text-center max-w-md animate-fade-up">
        ...
        <p className="eyebrow justify-center mt-6">{t('notFound.eyebrow')}</p>
        <h1 className="mt-2 font-display text-6xl font-semibold tracking-tight text-ink-900">404</h1>
        <p className="mt-2 text-ink-700">{t('notFound.text')}</p>
        <div className="mt-6 flex gap-2 justify-center">
          <Link to="/" className="btn-primary"><Home size={14} /> {t('notFound.home')}</Link>
          <button onClick={() => history.back()} className="btn-secondary"><ArrowLeft size={14} /> {t('notFound.goBack')}</button>
        </div>
      </div>
    </div>
  );
